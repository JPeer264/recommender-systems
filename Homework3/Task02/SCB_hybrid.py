...
def recommend_CF(UAM, seed_uidx, seed_aidx_train, K):
    ...
    new_dict_finish = {}
    counter = 0
    for key, value in new_dict_recommended_artists_idx.items():
        if counter < MIN_RECOMMENDED_ARTISTS and counter < len(sorted_dict_reco_aidx):
...
def run():
    ...
    for u in range(0, no_users):
        ...
        for train_aidx, test_aidx in kf:  # for all folds
            ...
            dict_rec_aidx_CF = recommend_CF(copy_UAM, u, u_aidx[train_aidx], K)
...
if __name__ == '__main__':
    ...
    UAM = np.loadtxt(UAM_FILE, delimiter='\t', dtype=np.float32)[:MAX_USER, :MAX_ARTISTS]
